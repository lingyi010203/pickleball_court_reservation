...
function Navbar() {
  ...
  useEffect(() => {
    const updateAuthState = () => {
      setIsLoggedIn(UserService.isLoggedIn() || UserService.isAdminLoggedIn());
      setUsername(UserService.getUsername() || UserService.getAdminUsername());
      setProfileImage(UserService.getProfileImage());
  ...
  const handleLogout = () => {
    if (isAdminRoute) {
      UserService.adminLogout(); // Clear admin tokens
    } else {
      UserService.logout(); // Clear regular user tokens
    }
